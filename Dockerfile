...
RUN echo -e "=========> download cmake" && cd /home && \
    wget -q https://github.com/Kitware/CMake/releases/download/v3.29.2/cmake-3.29.2-linux-x86_64.sh && \
...
RUN echo -e "=========> make protobuf" &&  cd /home && \
    ...
    cmake -S . -B build -Dprotobuf_BUILD_TESTS=OFF -DCMAKE_CXX_STANDARD=20 -DCMAKE_RULE_MESSAGES=OFF -DCMAKE_INSTALL_MESSAGE=NEVER && \
    cmake --build build && \
...
RUN echo -e "=========> make MySQL Connector/C++" && cd /home && \
    ...
    cmake -S . -B build -DWITH_PROTOBUF=system -DBUILD_STATIC=ON -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=ins_build -DCMAKE_RULE_MESSAGES=OFF -DCMAKE_INSTALL_MESSAGE=NEVER && \
    cmake --build build && \
...
RUN echo -e "=========> make hiredis" && cd /home && \
    ...
    cmake -S . -B build -DBUILD_SHARED_LIBS=OFF -DDISABLE_TESTS=ON -DCMAKE_RULE_MESSAGES=OFF -DCMAKE_INSTALL_MESSAGE=NEVER && \
    cmake --build build && \
    cmake --build build --target install && \
    rm -rf /home/hiredis
...
RUN echo -e "=========> make redis-plus-plus" && cd /home && \
    ...
    cmake -S . -B build -DREDIS_PLUS_PLUS_BUILD_SHARED=OFF -DREDIS_PLUS_PLUS_BUILD_TEST=OFF -DREDIS_PLUS_PLUS_CXX_STANDARD=17 -DCMAKE_RULE_MESSAGES=OFF -DCMAKE_INSTALL_MESSAGE=NEVER && \
    cmake --build build && \
    cmake --build build --target install && \
...
RUN echo "=========> install lsp-server: clangd" && cd /home &&\
    wget -q https://github.com/clangd/clangd/releases/download/17.0.3/clangd-linux-17.0.3.zip && \
