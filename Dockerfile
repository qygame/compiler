...
RUN echo -e "=========> make protobuf" &&  cd /home && \
    ...
    cmake -S . -B build -Dprotobuf_BUILD_TESTS=OFF -Dprotobuf_BUILD_SHARED_LIBS=ON -DCMAKE_CXX_STANDARD=20 -DCMAKE_RULE_MESSAGES=OFF -DCMAKE_INSTALL_MESSAGE=NEVER && \
    cmake --build build && \
...
RUN echo -e "=========> make MySQL Connector/C++" && cd /home && \
    ...
    cmake -S . -B build -DWITH_PROTOBUF=system -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=ins_build -DCMAKE_RULE_MESSAGES=OFF -DCMAKE_INSTALL_MESSAGE=NEVER && \
    cmake --build build && \
